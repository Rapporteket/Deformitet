...
RegData <- deformitet::preprosData(raw_regdata)
```
...
age_curve_data <- RegData |>
  ...
  dplyr::group_by(ShNavn, year = lubridate::floor_date(SURGERY_DATE, "year")) |>
  dplyr::mutate(year = format(year, "%Y")) |>
  dplyr::filter(year != lubridate::year(Sys.Date())) |>
  dplyr::summarize(
    n_opererte = as.character(dplyr::n_distinct(PasientID)),
...
key_data_table <- key_data |>
  dplyr::mutate(
    pro_kvinne = paste(pro_kvinne, "%", sep = ""),
...
# Få ønsket rekkefølge på ShNavn
key_data_table <- key_data_table |>
  dplyr::arrange(factor(ShNavn, levels = c("Nasjonalt", "Haukeland", "Rikshospitalet", "St.Olav"))) |>
  dplyr::filter(ShNavn == "Nasjonalt") # lagt til denne filtreringen siden reg.leder kun ønsker visning av nasjonalt
